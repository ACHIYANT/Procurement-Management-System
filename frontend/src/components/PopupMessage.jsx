...
const PopupMessage = ({
  ...
  useEffect(() => {
    queueMicrotask(() => setLocalOpen(open));
  }, [open]);

  useEffect(() => {
    queueMicrotask(() => setShowDiagnostics(Boolean(diagnostic?.defaultExpanded)));
  }, [diagnostic, open]);
  ...
  const handleClose = () => {
    ...
    closeWithDelay(() => {
      ...
      if (typeof moveTo === "string" && moveTo.trim()) {
        navigate(moveTo, moveState ? { state: moveState } : undefined);
      }
    });
  };
