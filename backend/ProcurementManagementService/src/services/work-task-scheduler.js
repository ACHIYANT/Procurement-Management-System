...
const DEFAULT_RUN_SECOND_OFFSET = 20;
...
const runSchedulerJob = async (label, task, fallback) => {
  try {
    ...
    console.error(
      `[WorkTaskScheduler] ${label} failed:`,
      error.message || error,
    );
    return fallback;
  }
...
const startWorkTaskScheduler = () => {
  if (
    String(process.env.WORK_TASK_SCHEDULER_DISABLED || "").toLowerCase() ===
    "true"
  ) {
    return null;
  }
  ...
  const intervalMinutes = Number(
    process.env.WORK_TASK_SCHEDULER_INTERVAL_MINUTES ||
  ...
  const run = async () => {
    const [syncResult, reminderResult, escalationResult, pushResult] =
      await Promise.all([
        runSchedulerJob(
          "system task sync",
